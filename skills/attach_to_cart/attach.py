...
###############################################################################
    ...
    async def avoid_obstacle(self):
        #### function activated if object or humen detected suddenly 
        # between the robot and cart
        # the function stops gary from moving, alert with beeping sound and 
        # apply sleep method for OBSTACLE_SLEEPING_TIME seconds.
        # after MAX_OBSTACLE_INDEX times of identification, 
        # the function abort the skill
      
        self.obstacle_index = self.obstacle_index + 1
        self.log.error(
            'stop moving, obstacle detected, '
            f'index: {self.obstacle_index}'
        )
    ...
    async def attach(self):
        ...
        try:
            while(True):
                ...
                if self.gripper_state['attempts'] > \
                    ATTEMPTS_BEFORE_VIBRATION and  \
                        self.gripper_state['attempts'] < MAX_ATTEMPTS -1:
                   await self.vibrate()
        ...
        try:
            ...
            for f in files_lst:
                ...
                if not check_file_exists(path_tmp):
                    ...
                    shutil.copyfile(
                        os.path.join(LOCAL_AUDIO_PATH, f), 
                        resolve_path(path_tmp)
                    )
                else:
